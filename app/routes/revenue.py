...
def getRevenueByBooking(booking_id):
    try:
        ...
        serviceOrders: ServiceOrders = ServiceOrders.query.filter_by(booking_id=booking_id).all()
...
def getTotalRevenueByDate():
    ...
    for booking in bookings:
        if date in booking.checkoutDate:
            checkoutBook.append(booking)
    revenuesDay = list(map(lambda x:getRevenueByBooking(x.id)['totalBill'], checkoutBook))
